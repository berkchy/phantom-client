...
static void UI_StripColorCodes( const char *in, char *out, size_t outSize )
{
	size_t oi = 0;
	...
	for( int si = 0; in[si] && oi + 1 < outSize; si++ )
	{
		if( IsColorString( in + si ))
		{
			si++;
			continue;
		}
...
{
	if( g_bImGuiTextEnabled && EngFuncs::textfuncs.pfnImGui_DrawText )
	{
		char plain[1024];
		const char *drawText = line;

		if( stripColors )
		{
			UI_StripColorCodes( line, plain, sizeof( plain ));
			drawText = plain;
		}
		...
		int width = 0;
		if( EngFuncs::textfuncs.pfnImGui_GetTextWidth )
			width = EngFuncs::textfuncs.pfnImGui_GetTextWidth( drawText, charH > 0 ? charH : g_flImGuiFontSize );

		EngFuncs::textfuncs.pfnImGui_DrawText( x, y, Red( color ), Green( color ), Blue( color ), Alpha( color ), drawText );
	...
	return EngFuncs::engfuncs.pfnDrawConsoleString( x, y, line );
}
		...
		int charH, uint justify, uint flags )
{
	...
	int maxX = x;
	float savedFontScale = 1.0f;
	bool menuFontScaled = false;

	if( ui_menufont_scale && ui_menufont_scale->value != 1.0f )
	{
		savedFontScale = EngFuncs::GetCvarFloat( "con_fontscale" );
		if( savedFontScale > 0.0f )
		{
			EngFuncs::CvarSetValue( "con_fontscale", savedFontScale * ui_menufont_scale->value );
			menuFontScaled = true;
		}
	}
	...
	{
		if( menuFontScaled )
			EngFuncs::CvarSetValue( "con_fontscale", savedFontScale );
		return x;
	}

	char cleanString[4096];
	UI_StripColorCodes( string, cleanString, sizeof( cleanString ));
	string = cleanString;
	...
	while( string[i] && !giveup )
	{
		...
		// draw it
		if( flags & ETF_FORCECOL )
		{
			// strip color codes for forced color
			char plain[1024];
			UI_StripColorCodes( line, plain, sizeof( plain ));

			if( flags & ETF_SHADOW )
				UI_DrawTextLine( xx + shadowOffset, yy + shadowOffset, plain, shadowModulate, charH, false );

			xx = UI_DrawTextLine( xx, yy, plain, modulate, charH, false );
		}
		else
		{
			if( flags & ETF_SHADOW )
				UI_DrawTextLine( xx + shadowOffset, yy + shadowOffset, line, shadowModulate, charH, true );

			xx = UI_DrawTextLine( xx, yy, line, modulate, charH, true );
		}
		maxX = Q_max( xx, maxX );
		yy += charH;
	...
	// Load ImGui font from fonts/font.ini if available
	{
		...
		else
		{
			...
			while( pos < end )
			{
				...
				{
					Q_strncpy( fontPath, value, sizeof( fontPath ));
					Con_Printf( "ImGui: font config: font = %s\n", fontPath );
				}
				else if( !stricmp( key, "size" ) )
				{
					fontSize = (float)atof( value );
					Con_Printf( "ImGui: font config: size = %.0f\n", fontSize );
				}
			}
