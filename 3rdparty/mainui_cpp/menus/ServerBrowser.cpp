...
void server_t::UpdateData( void )
{
	...
	// truncate long hostnames to prevent overlapping with Map column
	int maxNameLen = (int)(ui_hostname_limit ? ui_hostname_limit->value : 28.0f);
	if( strlen( name ) > maxNameLen )
	{
		Q_strncpy( name + Max( maxNameLen - 3, 0 ), "...", sizeof( name ) - Max( maxNameLen - 3, 0 ) );
	}
...
void CMenuGameListModel::Update( void )
{
	...
	// remove unreachable servers (ping >= 9999ms)
	for( i = servers.Count() - 1; i >= 0; i-- )
	{
		if( servers[i].ping >= MAX_PING - 0.001f )
			servers.FastRemove( i );
	}
...
void CMenuServerBrowser::RefreshList()
{
	...
	else if( uiStatic.realTime > refreshTime2 )
	{
		...
		refreshTime2 = uiStatic.realTime + 1000;
...
void CMenuServerBrowser::AddServer( void )
{
	...
	// FIXME: for now we can only show custom servers at Servers tab
...
{
	// remove unreachable servers every frame before drawing
	for( int i = gameListModel.servers.Count() - 1; i >= 0; i-- )
	{
		if( gameListModel.servers[i].ping >= MAX_PING - 0.001f )
			gameListModel.servers.FastRemove( i );
	}
...
void CMenuServerBrowser::_Init( void )
{
	...
	gameList.SetCharSize( QM_SMALLERFONT );
	gameList.SetupColumn( COLUMN_PASSWORD, NULL, 18.0f, true );
	gameList.SetupColumn( COLUMN_FAVORITE, NULL, 18.0f, true );
	gameList.SetupColumn( COLUMN_NAME, L( "Name" ), 0.30f );
	gameList.SetupColumn( COLUMN_MAP, L( "GameUI_Map" ), 0.25f );
	gameList.SetupColumn( COLUMN_PLAYERS, L( "Players" ), 82.0f, true );
	gameList.SetupColumn( COLUMN_PING, L( "Ping" ), 82.0f, true );
	gameList.SetupColumn( COLUMN_IP, L( "IP" ), 0.20f );
	gameList.SetModel( &gameListModel );
	gameList.bFramedHintText = false;
	gameList.bAllowSorting = true;
	gameList.iBackgroundColor = PackRGBA( 24, 24, 24, 200 );
	gameList.onChanged = VoidCb( &CMenuServerBrowser::OnChangeSelectedServer );
	gameList.SetSize( -20, 465 );

	tabSwitch.SetRect( 16, 16, -32, 34 );
	...
	tabSwitch.iSelectColor = PackRGBA( 220, 180, 60, 255 );
	tabSwitch.iBackgroundColor = PackRGBA( 30, 30, 30, 200 );
	tabSwitch.iFgTextColor = PackRGBA( 180, 180, 180, 255 );
	tabSwitch.iBgTextColor = PackRGBA( 80, 80, 80, 255 );
...
{
	refreshTime = uiStatic.realTime + 500;
	refreshTime2 = uiStatic.realTime + 500;

	const int margin = 22;
	const int fullW = size.w - margin * 2;

	// Tab switch at top
	tabSwitch.SetRect( margin, 14, fullW, 32 );

	// Server list fills remaining space above bottom buttons
	int tableY = 14 + 32 + 10;
	int bottomH = 100;
	int tableH = size.h - tableY - bottomH;
	gameList.SetCoord( margin, tableY );
	gameList.SetSize( fullW, tableH );
	gameList.VidInit();
...
void CMenuServerBrowser::LayoutButtons()
{
	const int margin = 22;
	const int btnH = 32;
	const int btnW = 130;
	...
	CMenuPicButton *visible[16];
	int visibleCount = 0;
	for( int i = 0; i < m_iBtnsNum && visibleCount < 16; i++ )
	{
		if( m_apBtns[i]->IsVisible())
			visible[visibleCount++] = m_apBtns[i];
	}

	if( visibleCount == 0 )
		return;

	int perRow = (visibleCount + 1) / 2;
	if( perRow > 4 ) perRow = 4;

	int row1Count = (perRow < visibleCount) ? perRow : visibleCount;
	int btnY = size.h - ((visibleCount > perRow) ? (btnH + gap + btnH + 12) : (btnH + 12));

	int x = margin;
	for( int i = 0; i < row1Count; i++ )
	{
		visible[i]->SetCoord( x, btnY );
		visible[i]->SetSize( btnW, btnH );
		visible[i]->CalcPosition();
		x += btnW + gap;
	}

	int remaining = visibleCount - row1Count;
	if( remaining > 0 )
	{
		btnY += btnH + gap;
		x = margin;
		for( int i = row1Count; i < visibleCount; i++ )
		{
			visible[i]->SetCoord( x, btnY );
			visible[i]->SetSize( btnW, btnH );
			visible[i]->CalcPosition();
