...
void CMenuBaseItem::ApplyMenuFontCvars()
{
	font = uiStatic.hConsoleFont;

	if( ui_menufont_scale )
	{
		int baseSize = 18;
		int size = (int)( baseSize * ui_menufont_scale->value );
		if( size > 0 && size != charSize )
		{
			charSize = size;
			CalcSizes();
		}
